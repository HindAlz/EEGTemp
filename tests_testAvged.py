...
def averaging(X_data, Y_data):
    ...
    for cond in range(4): #0, 1, 2, 3, for each condition
        ...
        cond_indices = np.arange(start, end) #array of indecies for this condition

        for i in range(start, end): #across all trials of this cond
            selected = np.random.choice(cond_indices, 5) #select 5 random and average
...
def losoModel_one_subject_all_configs(X_data_list, Y_data_list, subject_idx, cond1, cond2, num_classes, sfreq=200,
    ...
    # Apply averaging to test data
    X_test_raw, Y_test_avg = averaging(X_data_list[subject_idx], y_test)
    X_test_psd, _ = averaging(X_test_psd, y_test)  # Use same labels for both
    print(Y_test_avg.shape)
    print(Y_test_avg)

    # One-hot encode averaged labels
    y_test_cat = to_categorical(Y_test_avg, num_classes=2)
    ...
    # Apply averaging to training data
    X_train_raw, Y_train_avg = averaging(X_train_raw, y_train)
    X_train_psd, _ = averaging(X_train_psd, y_train)
    print("train", Y_train_avg.shape)
    print(Y_train_avg)
    input_shape_raw = X_train_raw.shape[1:]
    input_shape_psd = X_train_psd.shape[1:]

    # One-hot encode averaged labels
    y_train_cat = to_categorical(Y_train_avg, num_classes=2)
    ...
    for cfg in all_configs:
        ...
        config_name = f"heads{num_heads}_dim{key_dim}_denseSize{Dsize}_activ{act}, _avg{avg}, _attn{attn}_avgTestFIXED"
