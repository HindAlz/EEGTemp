...
# Directories & Files
base_dir = "codes/"  # Get script's directory
...
# ==================================================================================================
# LOAD DATA & PREPARE
data_path='processed_data_nt_st.pkl'
# Load Data
try:
    with open('processed_data_nt_st.pkl', 'rb') as f:
        subjects_X_data, subject_y_data = pickle.load(f)

    # Flatten data
    X = np.concatenate(subjects_X_data, axis=0)
    y = np.concatenate(subject_y_data, axis=0)

    subject_ids = []
    for subj_idx, X_subj in enumerate(subjects_X_data):
        n_samples = X_subj.shape[0]
        subject_ids.extend([subj_idx] * n_samples)
...
for i in range(start_subject_index, len(unique_subject_list)):
    ...
    if model_choice == 'ATCNet':
        ...
        model = ATCNet(nb_classes, X_train.shape[2], X_train.shape[3],  n_windows=3)
    #elif model_choice == 'EEGNet':
