...
def losoModel_one_subject_all_configs(X_data_list, Y_data_list, X_test_data, Y_test_data, subject_idx, cond1, cond2, num_classes, sfreq=200,
                                      avg=False):
    print("subject ", subject_idx)
    clear_tf_memory()

    #test data
    test_data = X_test_data[subject_idx].squeeze(1).transpose(1, 2, 0)
    X_test_psd = psd(test_data, fs=sfreq)
    X_test_psd = np.expand_dims(X_test_psd, axis=1)
    y_test = Y_test_data[subject_idx]
                    ...
                    for key_dim in key_dim_options:
                        for Dsize in dense_sizes:
                            for act in activations:
                                for attn in attn:
                                    config_name = f"heads{num_heads}_dim{key_dim}_denseSize{Dsize}_activ{act}, _avg{avg}, _attn{attn}_TCN_merge_Temporal_filefix_avgnnow"
