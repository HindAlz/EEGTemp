...
def threeBranch(input_shape_raw, input_shape_psd, num_classes, attn,
                ...
                num_heads=7, key_dim=10, dense_size=256, act="elu"):
...
def averaging(X_data, Y_data, avg_n):
    output = []
    labels = []
    trials = len(Y_data)
    start = 0 #first trial
    ...
    for cond in range(4): #0, 1, 2, 3, for each condition
        ...
        for i in range(start, end): #across all trials of this cond
            selected = np.random.choice(cls_indices, avg_n) #select 5 random and average
...
def losoModel_one_subject_all_configs(X_data_list, Y_data_list, avg_num, subject_idx, cond1, cond2, num_classes, sfreq=200,
                                      avg=False):
    ...
    y_test = np.array([0 if y in [0, 2] else 1 for y in y_test])
    ...
    for i in range(len(X_data_list)):
        if i == subject_idx:
            continue
        x, y = averaging(X_data_list[i], Y_data_list[i], avg_num)
        X_data_aug.append(x)
        Y_data_aug.append(y)

    X_train_raw = np.concatenate(X_data_aug, axis=0)
    y_train = np.concatenate(Y_data_aug, axis=0)
    y_train_cat = to_categorical(y_train, num_classes=2)

    psd_train_list = []
    for x in X_data_aug:
        d = x.squeeze(1).transpose(1, 2, 0)
                    ...
                    for key_dim in key_dim_options:
                        for Dsize in dense_sizes:
                            for act in activations:
                                for attn in attns:
                                    config_name = f"heads{num_heads}_dim{key_dim}_denseSize{Dsize}_activ{act}, _avg{avg}, _attn{attn}_avg{avg_num}_REMOVETCN"
...
for i in range (2, 10):
    for subj in range(half):
        ...
        accs, avg = losoModel_one_subject_all_configs(
            ...
            cond1="mi", cond2="non-mi", num_classes=2, avg_num=i
        )
